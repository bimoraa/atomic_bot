...
export async function update_work_report(
  ...
  if (existing) {
    ...
    await db.update_one(WORK_REPORTS_COLLECTION, { staff_id }, {
      staff_id,
      staff_name,
      total_work:           existing.total_work + 1,
      total_work_this_week: is_same_week ? existing.total_work_this_week + 1 : 1,
      total_salary:         existing.total_salary + salary,
      salary_this_week:     is_same_week ? existing.salary_this_week + salary : salary,
      week_number,
      year,
      last_work:            now,
    }, true)
