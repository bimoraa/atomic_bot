...
export async function load_priority_ticket(thread_id: string): Promise<boolean> {
  ...
  const data = await db.find_one<PriorityTicketData>(PRIORITY_TICKETS_COLLECTION, { thread_id })
  if (!data) return false
...
export async function load_all_priority_tickets(): Promise<void> {
  ...
  const tickets = await db.find_many<PriorityTicketData>(PRIORITY_TICKETS_COLLECTION, {})
  for (const data of tickets) {
